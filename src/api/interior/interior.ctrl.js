...
exports.pagenate = async (ctx) => {
    ...
    // NaN 오류 발생 함. id 에
    if(type === 'view'){
        const result = await interior.pageByView( pagenum, 2);
        ctx.body = {
            status : 200,
            result
        }
    }else if(type === 'date'){
        const result = await interior.pageByNew( pagenum, 2);
        ctx.body = {
            status : 200,
            result
        }
...
exports.search = async (ctx) => {
    ...
    const result = await interior.search(searchName[0],searchName[1],searchName[2]);
...
exports.create = async (ctx) => {
    const params = Joi.object({
        ...
        thumnail_image : Joi.string().require(),
